...
def window_area_partition(x, window_size):
    ...
    # view(): -> [B, d, Md, h, Mh, w, Mw, C]    Md = window size[0], Mh = window size[1], Mw = window size[2]
    x = x.view(B, D // window_size[0], window_size[0], H // window_size[1], window_size[1], W // window_size[2],
               window_size[2], C)
    # permute(): -> [B, d, h, w, Md, Mh, Mw, C]
    # view(): -> [B, d, h, w, Md*Mh*Mw, C]
    ...
    # [d+2, h+2, w+2, Md*Mh*Mw, C]
    a = F.pad(x, dim, "constant")

    # sb = Md*Mh*Mw
    sb = reduce(mul, window_size)

    # [num_windows, 27, Md*Mh*Mw, C]  num_windows = d*h*w, 27 = a*b*c, sb = Md*Mh*Mw
    b = torch.zeros(size=(B, d * h * w, 27, sb, C)).to(x.device)

    b[:, :, 0] = a[:, :-2, :-2, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 1] = a[:, :-2, :-2, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 2] = a[:, :-2, :-2, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 3] = a[:, :-2, 1:-1, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 4] = a[:, :-2, 1:-1, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 5] = a[:, :-2, 1:-1, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 6] = a[:, :-2, 2:, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 7] = a[:, :-2, 2:, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 8] = a[:, :-2, 2:, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 9] = a[:, 1:-1, :-2, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 10] = a[:, 1:-1, :-2, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 11] = a[:, 1:-1, :-2, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 12] = a[:, 1:-1, 1:-1, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 13] = a[:, 1:-1, 1:-1, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 14] = a[:, 1:-1, 1:-1, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 15] = a[:, 1:-1, 2:, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 16] = a[:, 1:-1, 2:, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 17] = a[:, 1:-1, 2:, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 18] = a[:, 2:, :-2, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 19] = a[:, 2:, :-2, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 21] = a[:, 2:, :-2, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 21] = a[:, 2:, 1:-1, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 22] = a[:, 2:, 1:-1, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 23] = a[:, 2:, 1:-1, 2:].reshape(B, d * h * w, sb, C)

    b[:, :, 24] = a[:, 2:, 2:, :-2].reshape(B, d * h * w, sb, C)
    b[:, :, 25] = a[:, 2:, 2:, 1:-1].reshape(B, d * h * w, sb, C)
    b[:, :, 26] = a[:, 2:, 2:, 2:].reshape(B, d * h * w, sb, C)

    # view(): -> [num_windows, 27*Md*Mh*Mw, C]
    b = b.view(-1, sb * 27, C)
    return b
...
class XMorpher(nn.Module):
    ...
    def __init__(self,
        ...
        self.reverse_patch_embedding = nn.ConvTranspose3d(2 * embed_dim, embed_dim // 2, kernel_size=patch_size,
                                                          stride=patch_size)

    def forward(self, moving, fixed):
        """Forward function."""

        rD, rH, rW = moving.shape[2:]
        ...
        if rD % self.patch_size[0] != 0 or rH % self.patch_size[1] != 0 or rW % self.patch_size[2] != 0:
            x = x[:, :, :rD, :rH, :rW].contiguous()
        return x


class XMorpherHead(nn.Module):
...
if __name__ == '__main__':
    ...
    Reger = XMorpherHead(n_channels=n_channels)

    # Reger = torch.nn.DataParallel(Reger, device_ids=[0,2,3])
